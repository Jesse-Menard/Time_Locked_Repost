...
bool CollisionManager::AABBCheck(SDL_FRect* rect, GameObject* object)
{
	// prepare relevant variables
	auto rp = glm::vec2(rect->x, rect->y);
	auto op = object->GetTransform()->position;
	const auto r_width = rect->w;
	const auto r_height = rect->h;
	const auto o_width = static_cast<float>(object->GetWidth());
	const auto o_height = static_cast<float>(object->GetHeight());

	// Is Centered checks
	if (object->isCentered())
	{
		op += glm::vec2(-o_width * 0.5f, -o_height * 0.5f);
	}
	...
	{
		if (!object->GetRigidBody()->isColliding)
		{

			object->GetRigidBody()->isColliding = true;
	...
	object->GetRigidBody()->isColliding = false;
	return false;
}
...
bool CollisionManager::LineAABBCheck(Agent* object1, GameObject* object2)
{
...
void CollisionManager::CircleCollisionPush(GameObject* object1, GameObject* object2, glm::vec2& mtv, bool push_from_object1)
{
	const glm::vec2 object1_position = glm::vec2(object1->GetTransform()->position.x, object1->GetTransform()->position.y + static_cast<float>(object1->GetHeight()) / 2.0f);
	const glm::vec2 object2_position = glm::vec2(object2->GetTransform()->position.x, object2->GetTransform()->position.y + static_cast<float>(object2->GetHeight()) / 2.0f);
	const glm::vec2 delta = object2_position - object1_position;
	const float radiiSum = object1->GetWidth() / 2 + object2->GetWidth() / 2;
	const float centerDistance = Util::Distance(object1_position, object2_position);
	const bool collision = centerDistance <= radiiSum;

	if (collision)
	{
		// Compute mtv (Sum of radii - distance between centers)
		const float mtvDistance = radiiSum - centerDistance;
		mtv = Util::Normalize(delta) * mtvDistance;

		if(push_from_object1)
			object2->GetTransform()->position += mtv;
		else
		{
			object1->GetTransform()->position -= mtv / 2.0f;
			object2->GetTransform()->position += mtv / 2.0f;
		}
	}
}
