...
void UIControl::OnMouseOver(const bool centered)
{
	if (const auto mouse_position = EventManager::Instance().GetMousePosition(); 
		CollisionManager::PointRectCheck( mouse_position,  centered ? GetTransform()->position : GetTransform()->position + glm::vec2(GetWidth() / 2, GetHeight() / 2), static_cast<float>(GetWidth()), static_cast<float>(GetHeight())))
	{
		m_mouseOver = true;
	}
	else
...
void UIControl::OnMouseOut(const bool centered)
{
...
void UIControl::OnLeftMouseButtonClick(const bool centered)
{
